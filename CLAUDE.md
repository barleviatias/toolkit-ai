...
**Current coverage (54 tests):** item-key, lock (read/write/protect/record), catalog (frontmatter/hash/find), fs-helpers (copy/link/remove), scanner (RCE/shells/size/IPs/URLs/format + interpreter-pipe variants, reverse-shell variants, inline exec, base64-decoded exec, script-file scanning, mcp-stdio-exec), codex config round-trip, install/recovery/removal, security paths, strict-mode install gate, plugin decompose install + remove round-trip across every detected provider (both `.claude-plugin/plugin.json` and generic top-level `plugin.json` manifests), discovery + cross-provider install of plugins already brought down by Claude Code's `/plugin install` and GitHub Copilot CLI's `copilot plugin install` (synthetic `claude` and `copilot` sources), recursive component discovery driven by manifest path overrides (`skills` / `agents` / `commands` / `mcps` arrays in `plugin.json`) for cross-tool plugin packages like AMS that nest content under per-tool adapter dirs, native Copilot install registration (config.json + settings.json + cache tree) so plugins surface in Copilot's Plugins UI, and an operation log at `~/.toolkit/log.jsonl` with a `toolkit logs` tail command.
